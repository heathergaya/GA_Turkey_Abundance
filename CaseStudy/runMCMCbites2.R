...
runMCMCbites <- function( mcmc,
  ...
  for(nb in 1:bite.number){
    ...
    #waic_now <- as.list(Cmcmc$getWAIC())
...
collectMCMCbites <- function( path,
  ...
  numSamples <- unlist(lapply(res,nrow))
  minSamples <- min(numSamples)
  if(any(numSamples > minSamples)){
  ...
  if(sp2yes){
    numSamples2 <- unlist(lapply(res2,nrow))
    minSamples2 <- min(numSamples2)
    if(any(numSamples2 > minSamples2)){
